...
def check_native_rom(netlist_path: Path, reference_path: Path, generated: Path, top: str = "mkTop",
                     clock_net: str = "clocks_coreReset.CLK") -> dict:
    ...
    for key in sorted(matched):
        ...
        address_cells = []
        for bit, output_bit in enumerate(actual_address):
        ...
        results.append({"layer": layer, "lane": lane, "cell": name, "source": filename,
                        ...
                        "address_mapping_checked": "command.D_IN19:9 = addressCnt10:0; FIFO1 FF DI=counter, Q=ADA13:3; ADA2:0=0"})
    require(len(clocks) == 1, "Coefficient banks do not share one clock signal")
    require(not coefficient_alias_cells - address_registers,
            f"Unproven cells in coefficient hierarchy: {sorted(coefficient_alias_cells - address_registers)}")
    require(len(address_registers) == 11 * 11, "Expected exactly eleven shared command address registers per engine")
    return {"status": "pass", "evidence": "raw flattened Yosys native coefficient memory netlist audit",
            ...
            "coefficient_alias_registers_checked": sorted(coefficient_alias_cells),
