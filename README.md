...
| Packed logic, `TRELLIS_COMB` | 44,898 | 45,987 | 45,989 |
...
| Core post-route Fmax / 100 MHz constraint | 101.49 MHz / PASS | 92.19 MHz / FAIL; targeted buffer check pending | Not measured |
...
For a clock conversion after successful routing, rerun `check_perf.py` with that same configuration's `--timing "$obs_results/physical/timing.json"` and selected `--linear-source` package, and omit `--cycles-only`. B0's timing report cannot establish B1/B2 timing.
